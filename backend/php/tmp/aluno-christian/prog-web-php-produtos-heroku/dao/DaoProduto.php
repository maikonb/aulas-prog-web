...
class DaoProduto {
  ...
  public function porId(int $id): ?Produto {
    $sql = "SELECT produtos.nome, produtos.preco, 
                   ...
                   produtos.fornecedor_id, fornecedores.nome, fornecedores.endereco
            FROM produtos 
            LEFT JOIN marcas ON marcas.id = produtos.marca_id
            LEFT JOIN fornecedores ON fornecedores.id = produtos.fornecedor_id
            WHERE produtos.id = ?";
    ...
    if ($stmt) {
      ...
      if ($stmt->execute()) {
        $stmt->bind_result($nome, $preco, $estoque, $marca_id, $marca_nome, $fornecedor_id, $fornecedor_nome, $fornecedor_endereco);
        $stmt->store_result();
        if ($stmt->num_rows == 1 && $stmt->fetch()) {
          $prod = new Produto($nome, $preco, $estoque, 
            new Marca($marca_nome, $marca_id), 
            new Fornecedor($fornecedor_nome, $fornecedor_endereco, $fornecedor_id), $id);
        }
      }
      $stmt->close();
    }
  ...
  public function inserir(Produto $produto): bool {
    $sql = "INSERT INTO produtos (nome, preco, estoque, marca_id, fornecedor_id) VALUES(?,?,?,?,?)";
    $stmt = $this->connection->prepare($sql);
    $res = false;
    if ($stmt) {
      ...
      $stmt->bind_param('sdiii', $nome, $preco, $estoque, $marca_id, $fornecedor_id);
      if ($stmt->execute()) {
  ...
  public function atualizar(Produto $produto): bool {
    $sql = "UPDATE produtos SET nome=?, preco=?, estoque=?, marca_id=?, fornecedor_id=? WHERE id = ?";
    $stmt = $this->connection->prepare($sql);
    $ret = false;      
    if ($stmt) {
      $nome = $produto->getNome();
      ...
      $fornecedor_id = $produto->getFornecedor()->getId();    
      $id   = $produto->getId();
      $stmt->bind_param('sdiii', $nome, $preco, $estoque, $marca_id, $fornecedor_id, $id);
      $ret = $stmt->execute();
      $stmt->close();
    }
    return $ret;
  ...
  public function todos(): array {
    ...
    if ($stmt) {
      if ($stmt->execute()) {
        $id = 0; $nome = '';
        $stmt->bind_result(
          $id, $nome, $preco, $estoque, $marca_id, $marca_nome, $fornecedor_id, $fornecedor_nome, $fornecedor_endereco
        );
        $stmt->store_result();
        while($stmt->fetch()) {
          ...
          $fornecedor = new Fornecedor($fornecedor_id, $fornecedor_nome, $fornecedor_endereco);
          $produtos[] = new Produto($nome, $preco, $estoque, $marca, $fornecedor, $id);
        }
      }
      $stmt->close();
    }
